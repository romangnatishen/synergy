<template>
  <div>
    <CCard>
      ...
      <CCardFooter>
        <CButton @click="createUser()" type="submit" size="sm" color="success"><CIcon name="cil-check-circle"/> Sapisz zmiany</CButton>
        <CButton @click="goBack()" type="reset" size="sm" color="info"><CIcon name="cil-ban"/> Cofnij</CButton>
...
async removeAwizo() {
  ...
  if (this.id > 0) {
    await this.$store.dispatch("profile/remove", awizoAddParams).then((result) => {
