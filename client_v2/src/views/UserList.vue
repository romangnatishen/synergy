      ...
      <CCol sm="12">
        <CCard>
          ...
          <template #user_status="{item}">
            <td>
              <CBadge :color="getBadge(item.user_status)">{{getStatusDescription(item.user_status)}}</CBadge>
            </td>
          </template>
            <template #email="{item}">
              <td class="py-2">
...
export default {
  ...
  methods: {
    getBadge (status) {
      return status === 'Aktywny' ? 'success'
        : status === 'Zablokowany' ? 'danger' : 'primary'
    },

    getStatusDescription (status) {
      return status === 'Aktywny' ? 'Aktywny'
        : status === 'Zablokowany' ? 'Zablokowany' : 'Brak statusu'
    },
    ...
    if (currentProfile) {
      ...
      if (this.dataArray) {
        for(let item of this.dataArray) {
          ...
          const statusResult = this.user_status_list.find(curStatus => curStatus.value === item.user_status);            
          if (statusResult) {
