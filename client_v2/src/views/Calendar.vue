  ...
  <CCard>
    ...
    <CCardBody>
      ...
      <template #footer>
        ...
        <CButton @click="cancelEventModification" color="danger">Cofnij</CButton>
      </template>
    </CModal>

    <CCardHeader>
      <CForm inline>
        <CSelect class="header-items"
        ...
        if (projectData) {
          projectData.forEach(el => {
            el.forEach(projectItem => {
              this.projectList.push({value:projectItem.id, label:projectItem.name});
            })
          });
        }
      },
      ...
      async saveEvent() {
        ...
        const eventData = {
          ...
          if (this.currentEvent>0) {          
            resEvent = await this.$store.dispatch("calendar/updateEvent", eventData);
          } 
          else { 
            resEvent = await this.$store.dispatch("calendar/createEvent", eventData);        
          }
          
          if (resEvent.data?.id) {
            let curStartDate = Date.parse(this.dateFrom) + Number(this.start_hours)+3600; 
            if (this.start_minutes != '') {
              curStartDate = curStartDate + Number(this.start_minutes)*60;
            }          
            let curEndDate = Date.parse(this.dateTo)+ Number(this.end_hours)+3600; 
            if (this.end_minutes != '') {
              curEndDate = curEndDate + Number(this.end_minutes)*60;
            }
            const resultArray = this.eventsArray.filter(el => el.id != resEvent.data.id);
