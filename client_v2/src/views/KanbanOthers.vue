  ...
  <div>
    ...
    <CCard>
      <CCardBody v-if="kanban_data.length>0">
        <CTabs>
          <CTab v-for="column in kanban_data"         
            :title="column.title" 
            :key="column.id">
            <template slot="title">
              {{column.title}}
              <CBadge color="primary" shape="pill"><strong>{{column.tasks.length}}</strong></CBadge>
            </template>            
            <CDataTable
              :items="column.tasks"
              :fields="taskFields"
...
export default {
  ...
  async makeChangesInRedmine(issueData) {
    ...
    async toggleDetails(issue) {
      ...
      if ((issue.executor_id===currentRedmineUser.data.user.id||issue.auditor_id===currentRedmineUser.data.user.id)&&issue.kanban_status_id===1) {
