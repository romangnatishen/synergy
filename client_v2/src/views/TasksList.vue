        ...
        <template #details="{item}">
          ...
          <CCardBody>
            <CRow>
              ...
              <!-- <CRow > -->
                <p></p>
                <p class="display:flex text-muted"><b>Opis:</b> {{item.description}}</p>
                <CTextarea 
...
methods: {
  ...
  async initialize() {
    if (this.$route.params) {
      ...
      const storeFilterProject = this.$store.getters['projects/getFilterProject'];
    ...
    if (dataObject) {
      ...
      dataObject.forEach(str => {
        str.forEach(el => {
          ...
          let moduleValue = "";
          if (el.custom_fields) {
            const moduleField = el.custom_fields.find(arrItem => arrItem.id === 12);
  ...
  //   },
    ...
    async addToKanban (item) {
      const foundExecutor = this.issue_executors.find(el => el.value===item.assigned_Id);
      const addData = {
      ...
      const presentInKanban = item.kanbanArray.filter(el => 
        Number(el.value) === Number(item.assigned_Id)
      );

      if (presentInKanban.length===0) {        
        item.kanbanArray = item.kanbanArray.filter(arrItem => arrItem.value!=0);
        const addValue = {
    ...
    async saveTaskChanges(item) {
      ...
      await this.$store.dispatch("issues/updateIssue", updateData).then(async ()=>{
